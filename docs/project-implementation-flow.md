...
- 明确指定图片：例如“形状见图1、图案见图2、排版见参考图3”，系统会把原文和解析出的职责绑定到对应公司设计参考图。图片级职责同时进入素材层级判断和真正的素材生成请求，不允许不同图片职责混用。
- 未指定图片：例如只写“可爱风格、紫色底、文案 Happy Halloween”，系统不会虚构图1/图2职责。AI 根据全部可用设计参考图、主要图案元素、文字元素和全局开发思路判断是否拆分及如何提取。

尺寸、规格和载体信息仍不作为图案素材提取目标；图片级要求只决定从输入图中已有内容提取什么，不允许据此凭空重绘新素材。
