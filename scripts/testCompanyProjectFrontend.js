...
function assertSourceIsSlim() {
  ...
  assert(componentSource.includes('categoryMaterialGroups'));
  assert(componentSource.includes('handleRefineMaterialImages(true, target.category, false, true)'));
  assert(componentSource.includes('splitDesignReferenceImages(target.category, false, true)'));
  assert(componentSource.includes('本轮素材板只服务于该品类的完整设计任务'));
  assert(componentSource.includes('已按 ${generationCategoryTargets.length} 个品类分别完成图库素材处理'));
  assert(componentSource.includes('已按 ${generationResponses.length} 个品类分别完成最终生图'));
  assert(componentSource.includes('`${group.target.category} AI 空版式母版提取`'));
  assert(componentSource.includes('category: categoryOverride'));
  assert(componentSource.includes('AI空版式母版-${categoryOverride}'));
  assert(componentSource.includes('materialImagesForCategory'));
  ...
  assert(componentSource.includes('reverseAndRegenerateDesignReferenceMaterial'));
  assert(componentSource.includes('material_board_reverse_source'));
  assert(componentSource.includes("request_mode: 'images'"));
  assert(componentSource.includes('文生图生成一张可用于后续图案设计的新${materialKind}'));
  assert(componentSource.includes("generationSource: 'design_reference_split_regeneration'"));
  assert(componentSource.includes('逐张反推视觉描述并使用纯文本重新生成'));
